...
class MixtureDensityEstimatorFromCorr(nn.Module):
    def __init__(self, in_channels, betas=[2, 480], output_channels=4, upsample=False, final_image_size=(480, 640)):
    ...
    def forward(self, x, flow, bhw):
        ...
        flow[:, 0, ...] =  flow[:, 0, ...] / self.final_image_size[0]
        flow[:, 1, ...] =  flow[:, 1, ...] / self.final_image_size[1]
        ...
        uncertainty[:, 1] = self.betas[0] + (self.betas[1] - self.betas[0]) * torch.sigmoid(uncertainty[:, 1])
        return uncertainty    
