...
class GMFlow(nn.Module):
    ...
    def extract_feature(self, img0, img1):
        ...
        for i in range(len(features)):
            ...
            feature0.append(chunks[0]) #for the fist image
            feature1.append(chunks[1]) #for the second image
    ...
    def forward(self, img0, img1,
        ...
        for scale_idx in range(self.num_scales):
            ...
            if scale_idx > 0: #upsampling of calculated flow
            ...
            if corr_radius == -1:  # global matching
                if self.probabilistic:
                    flow_pred, global_corr = global_correlation_softmax(feature0, feature1, pred_bidir_flow)
                    results_dict['global_corr'] = global_corr
                flow_pred, _ = global_correlation_softmax(feature0, feature1, pred_bidir_flow)
                
            else:  # local matching
                if self.probabilistic:
                    flow_pred, local_corr = local_correlation_softmax(feature0, feature1, corr_radius)
                    results_dict['local_corr'] = local_corr #B x HW x K x K
                flow_pred, _ = local_correlation_softmax(feature0, feature1, corr_radius)
            
            # flow or residual flow
            flow = flow + flow_pred if flow is not None else flow_pred
            
            # upsample to the original resolution for supervison
            if self.training and (not self.fine_tuning):  # only need to upsample intermediate flow predictions at training time
            ...
            if self.training and (scale_idx < self.num_scales - 1) and (not self.fine_tuning):
