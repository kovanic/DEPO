        ...
        for i in range(len(depths)):
            if i == 0:
                ...
                self.patch_embeds.append(
                    PatchEmbed(
                      # CHANGES HERE
                      (img_size[0] // patch_size // 2 ** (i - 1), 
                       img_size[1] // patch_size // 2 ** (i - 1)),
                      2, embed_dims[i - 1], embed_dims[i]))
...
@register_model
def pcpvt_small_v0_partial(pretrained=False, **kwargs):
    model = CPVTV2(
        patch_size=4, embed_dims=[64, 128], num_heads=[1, 2], mlp_ratios=[8, 8], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4], sr_ratios=[8, 4],
        **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def pcpvt_base_v0_partial(pretrained=False, **kwargs):
    model = CPVTV2(
        patch_size=4, embed_dims=[64, 128], num_heads=[1, 2], mlp_ratios=[8, 8], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 4], sr_ratios=[8, 4],
        **kwargs)
...
@register_model
def pcpvt_large_v0_partial(pretrained=False, **kwargs):
    model = CPVTV2(
        patch_size=4, embed_dims=[64, 128], num_heads=[1, 2], mlp_ratios=[8, 8], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[3, 8], sr_ratios=[8, 4],
        **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def alt_gvt_small_partial(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4, embed_dims=[64, 128], num_heads=[2, 4], mlp_ratios=[4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2], wss=[10, 10], sr_ratios=[8, 4],
        **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def alt_gvt_base_partial(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4, embed_dims=[96, 192], num_heads=[3, 6], mlp_ratios=[4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2], wss=[10, 10], sr_ratios=[8, 4],
        **kwargs)
    model.default_cfg = _cfg()
    return model
...
@register_model
def alt_gvt_large_partial(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4, embed_dims=[128, 256], num_heads=[4, 8], mlp_ratios=[4, 4], qkv_bias=True,
        norm_layer=partial(nn.LayerNorm, eps=1e-6), depths=[2, 2], wss=[10, 10], sr_ratios=[8, 4],
        **kwargs)
    model.default_cfg = _cfg()
    return model